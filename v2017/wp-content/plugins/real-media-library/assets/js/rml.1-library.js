...
window.rml.library = {
    ...
    initializeToolbar: function() {
    	...
    	wp.media.view.AttachmentFilters.RML = wp.media.view.AttachmentFilters.extend({
    	    ...
    	    createFilters: function() {
    			...
    			window.rml.hooks.call("toolbarFolderFilter", [ filters ], this);
    	        ...
    	        // Set default
    	        if (window.rml.hooks.exists("toolbarFolderFilterDefault")) {
    	        	window.rml.hooks.call("toolbarFolderFilterDefault", [ filters ], this);
    	        }else{
    ...
    initializeTreeContentLoader: function(selectedId) {
        return this.loadTreeContent(selectedId).done(function(response) {
            if (response.success) {
                // Names slug array
    ...
    showShortcutInfoAppendTo: function(rmlContainer, html) {
        ...
        (mediaSidebar.size() > 0 ? mediaSidebar :
        	(attachmentDetails.size() > 0 ? attachmentDetails : rmlContainer))
...
window.rml.hooks.register("afterInit/modal", function(menu, modal, $) {
    ...
    if (container.hasClass("aio-theme-wordpress-fixed") && fixedHeader.size() > 0 && fixedHeaderHelper.size() > 0) {
        menu.scroll(function() {
            ...
            console.log(reachedOverflow > toReach);
        });
    }
});
